...
public class ActionConfirmHarvesterHoe extends ChestActionAbstract {
    ...
    public boolean onClick(InventoryClickEvent event) {
        ...
        if (level == Levels.get().maxHarvesterHoeLevel) {
            MixinMessage.get().msgOne(player, MConf.get().maximumLevel);
            player.closeInventory();
            return false;
        }
        int newLevel = level + levelAmount;
        if (newLevel > Levels.get().maxHarvesterHoeLevel) newLevel = Levels.get().maxHarvesterHoeLevel;
        int price = 0;
        for (int i = level; i <= newLevel; i++) price += Levels.get().getPriceByLevel(i, ToolType.HARVESTER_HOE);
