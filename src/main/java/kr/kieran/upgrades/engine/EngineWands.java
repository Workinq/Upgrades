...
import gg.halcyon.outpost.engine.EngineOutpost;
import gg.halcyon.outpost.entity.Outpost;
...
public class EngineWands extends Engine {
    ...
    public void onUseCraftWand(PlayerInteractEvent event) {
        ...
        if (event.getItem().getType() != Tools.get().craftWandMaterial) return;
        if (!event.getItem().hasItemMeta()) return;
        if (!event.getItem().getItemMeta().hasDisplayName()) return;
        if (!event.getItem().getItemMeta().hasLore()) return;
        if (!event.getItem().getItemMeta().getDisplayName().equals(Txt.parse(Tools.get().craftWandName))) return;
        ...
        if (!BoardColl.get().getFactionAt(PS.valueOf(block.getLocation())).getId().equals(mPlayer.getFaction().getId())) {
        ...
        try {
            ...
            int diamondBlocks = diamonds / 9;
            int diamondOverflow = diamonds % 9;
            int emeraldBlocks = emeralds / 9;
            int emeraldOverflow = emeralds % 9;
            int ironBlocks = iron / 9;
            int ironOverflow = iron % 9;
            int goldBlocks = gold / 9;
            int goldOverflow = gold % 9;
            int goldIngots = goldNugget / 9;
            int nuggetOverflow = goldNugget % 9;
            int glowstoneBlocks = glowstone / 9;
            int glowstoneOverflow = glowstone % 9;
            int redstoneBlocks = redstone / 9;
            int redstoneOverflow = redstone % 9;
            int lapisBlocks = lapis / 9;
            int lapisOverflow = lapis % 9;
            int coalBlocks = coal / 9;
            int coalOverflow = coal % 9;
    ...
    public void onUseLightningWand(PlayerInteractAtEntityEvent event) {
        ...
        if (player.getItemInHand().getType() != Tools.get().lightningWandMaterial) return;
        ...
        if (!player.getItemInHand().getItemMeta().getDisplayName().equals(Txt.parse(Tools.get().lightningWandName))) return;
        event.setCancelled(true);
    ...
    public void onUseSandWand(PlayerInteractEvent event) {
        ...
        MPlayer mPlayer = MPlayer.get(player);
        if (event.getItem() == null) return;
        if (event.getItem().getType() != Tools.get().sandWandMaterial) return;
        if (!event.getItem().hasItemMeta()) return;
        if (!event.getItem().getItemMeta().hasDisplayName()) return;
        if (!event.getItem().getItemMeta().hasLore()) return;
        if (!event.getItem().getItemMeta().getDisplayName().equals(Txt.parse(Tools.get().sandWandName))) return;
        event.setCancelled(true);
        Block clickedBlock = event.getClickedBlock();
        if (!BoardColl.get().getFactionAt(PS.valueOf(clickedBlock)).getId().equals(mPlayer.getFaction().getId())) {
            MixinMessage.get().msgOne(player, MConf.get().notAllowedHere);
            return;
        }
        ...
        for (int y = clickedBlock.getLocation().getBlockY(); y > 0; y--) {
            ...
            if (current.getType() != Material.SAND) break;
            blocks.add(current);
        }
    ...
    public void onSellChest(PlayerInteractEvent event) {
        ...
        Player player = event.getPlayer();
        if (player.getItemInHand() == null) return;
        if (player.getItemInHand().getType() != Tools.get().sellWandMaterial) return;
        if (!player.getItemInHand().hasItemMeta()) return;
        if (!player.getItemInHand().getItemMeta().hasDisplayName()) return;
        if (!player.getItemInHand().getItemMeta().hasLore()) return;
        if (!player.getItemInHand().getItemMeta().getDisplayName().equals(Txt.parse(Tools.get().sellWandName))) return;
        ...
        if (!BoardColl.get().getFactionAt(PS.valueOf(clicked.getLocation())).getId().equals(mPlayer.getFaction().getId())) {
            MixinMessage.get().msgOne(player, MConf.get().notAllowedHere);
            return;
        }
        ...
        for (int i = 0; i < inventory.getSize(); i++) {
            ...
            if (item == null) continue;
            if (ShopGuiPlusApi.getItemStackPriceSell(player, item) == -1.0) continue;
        ...
        for (Map.Entry<ItemStack, Integer> map : sellItems.entrySet()) {
            ...
            if (org.jdgames.mobextras.entity.MConf.get().josephOutpostMultiplier && EngineOutpost.get().isActive()) {
                if (Outpost.get().outpostOwnerId != null && Outpost.get().outpostOwnerId.equals(mPlayer.getFaction().getId())) money = money * gg.halcyon.outpost.entity.MConf.get().sellMultiplier;
            }
            totalMoney += money;
    ...
    private boolean isInventoryEmpty(Inventory inventory) {
        for (ItemStack item : inventory.getContents()) if (item != null) return false;
        return true;
    }
